CREATE OR REPLACE FUNCTION fn_crear_especialidades(
    p_nombre VARCHAR
) RETURNS TABLE (
    exito BOOL,
    mensaje VARCHAR(1000),
    id_registro INT  -- Cambiado a INT
) AS $$
DECLARE
    v_exito BOOL := true;
    v_mensaje VARCHAR(1000) := 'Operación Exitosa';
    v_id_registro INT;  -- Cambiado a INT
BEGIN
    ...
    VALUES ('Se crea la especialidad ' || p_nombre);  -- Mensaje corregido
